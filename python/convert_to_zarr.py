...
print('COVERSION TO XARRAY FOR PLUME RUNS')
print('===================================')

for nCase,Case in enumerate(loc_tag):
    print('Examining case: %s ' %Case)
    #----------check for converted data----------------------------
    zarrpath = plume.wrfdir + 'interp/zarr/cs_'+ Case

    if os.path.isfile(zarrpath):
        print('Converted data found at: %s' %zarrpath)
        zarrfile = open(zarrpath,'rb')
        #PLUME = pickle.load(zarrfile)   # load here the above pickle

    else:
        print('WARNING: no converted data found - generating...')
        wrfcspath = plume.wrfdir + 'interp/wrfcs_' + Case + '.npy'
        ds = np.load(wrfcspath).item()
        dszarr = {}
        for key in ds.keys():
            if key=='temp' or key=='u' or key=='w' or key=='pm25':
                dszarr[key] = {'dims': ('time','z','x'), 'data': ds[key] }
                print(key)
            elif key=='ghfx':
                dszarr[key] = {'dims': ('time','x'), 'data': ds[key] }
                print(key)
            elif key=='ghfx2D':
                dszarr[key] = {'dims': ('time','y','x'), 'data': ds[key] }
                print(key)
        PLUME = xr.Dataset.from_dict(dszarr)
        #PLUME = xr.open_dataset(wrfpath)
        PLUME.to_zarr(zarrpath)
'''
